@props(['id','name','value','type','label'])
@if ($id && $name && $type && $label)
    <div class="relative h-10 w-full min-w-[200px]">
        <input
            {{$attributes->merge(['class' => 'peer h-full w-full rounded-[7px] border border-black border-t-transparent bg-transparent px-3 py-2.5 text-sm font-normal text-black outline outline-0 transition-all placeholder-shown:border placeholder-shown:border-black placeholder-shown:border-t-black focus:border-2 focus:border-black focus:border-t-transparent focus:outline-0 disabled:border-0 disabled:bg-black'])}}
