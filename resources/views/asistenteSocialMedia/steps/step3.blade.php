...
<div id="step-3-container">
    <div id="step-3-form-content">
        {{-- <form id="step-3-form" method="POST" action="{{route('herramienta1.rellenariasave')}}" data-validate="true"> --}}
            {{-- @csrf --}}
            <form id="step-3-form" method="POST" action="{{route('asistenteSocialMedia.guardar')}}" data-validate="true">
            @csrf
             <input type="hidden" name="account" id="account_id_step3" value="">
            <div id="asistenteSocialMediaGenerateContainer"></div>
            <input type="hidden" name="asistenteSocialMediaGenerateInput" id="asistenteSocialMediaGenerateInput"> 
            <x-dynamic-form 
                :fields="[
